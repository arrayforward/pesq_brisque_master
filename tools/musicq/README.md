...
- **chirp 导频**：默认 10–14 kHz、60 ms、峰值 -24 dBFS、每 5 s 一个（**standard 预设**，
  数字通道 scrcpy/A2DP 适用）；该频段高于流行乐主要能量区、又在常见编码器通带内。
  偶数序号标记上扫、奇数下扫，检测时按奇偶交叉校验，降低网格匹配歧义。
  **声学通道（扬声器→房间→麦克风）请用 acoustic 预设重新生成**
  （`gen --profile acoustic`：3-7 kHz、150 ms、-16 dBFS）——10-14kHz 在声学链路上有
  剧烈滚降（仿真模型 8kHz 截止，实测检出率 <50%），3-7kHz 落在有效响应内且更长
  chirp 有更高处理增益（声学仿真 -30~-50dB 噪声下检出率 100%）。
- **leader（chirp 簇）**：**歌头一个 + 正文每隔 30 s 一个**（6 个 chirp、150 ms 等间隔、
  交替扫频），与正文 5 s 间隔的常规标记在节奏上明显区分；与簇过近（<1s）的正文标记
  自动让位跳过。参数可调：`--leader-n` / `--leader-gap-ms` / `--leader-gap-after` / `--leader-interval`。
  markers.json v3 起含 `leaders`（全部簇位置）与 `song_id` 字段；旧版 json 仍可用于 `align`。
- **检测是两阶段的**：先带通（f0−300 ~ f1+300 Hz）+ 归一化匹配滤波（低阈值 MAD 自适应，
  假峰由"每标记间隔窗口 top-N 相关峰"预筛选控制）做粗检；再由**网格对齐 DP**（动态规划，
  间隔比率硬约束 + 匹配奖励，假峰海里全局最优）粗对应到参考标记；最后**按局部速率拉伸
  过的模板**做局部相关精修——裸模板在时间伸缩下有与扫频方向相关的系统偏差
  （实测 r=1.03 时上扫 +7ms / 下扫 −4ms），伸缩匹配模板可将其消到 0.2ms 以内。
  假峰匹配（rate=1.0 假网格）由 top-K 互异 DP 假设 + 分块内容验证消歧；
  精修质量低的假峰用大窗口重试仍失败则剔除。
