...
**声学通道请先用 acoustic 预设重新生成测试音频**（`gen --profile acoustic`）：
10-14kHz 的 standard chirp 在扬声器/房间/麦克风链路上有剧烈滚降（实测检出率 <50%），
acoustic 预设（3-7kHz、150ms、-16dBFS）落在有效响应内，仿真检出率 100%。
